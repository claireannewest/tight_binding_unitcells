...
class BtileL(RobinsonTriangle):
    ...
    def resize(self, size):
        ...
        """
        OA, OB, OC = self.A - self.centre(), self.B - self.centre(), \
                     self.C - self.centre()
        return BtileL(self.centre() + OA * size, self.centre() + OB * size,
                    self.centre() + OC * size)
...
class BtileS(RobinsonTriangle):
    ...
    def resize(self, size):
        ...
        OA, OB, OC = self.A - self.centre(), self.B - self.centre(), \
                     self.C - self.centre()
        return BtileS(self.centre() + OA * size, self.centre() + OB * size,
                    self.centre() + OC * size)
...
class PenroseP3:
    ...
    def make_tiling(self):
        ...
        new_elements = []
        for element in self.elements:
            new_elements.append(element.resize(self.config['size']))
        self.elements = new_elements
