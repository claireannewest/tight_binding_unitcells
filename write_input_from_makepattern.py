...
coordinates = np.loadtxt('parameter_files/coords_dimer.txt') #outputs of make_pattern, the (x,y) of the center of each rhombus
base_vectors = np.loadtxt('parameter_files/basevecs_dimer.txt') #outputs of make_pattern

coordinates = np.concatenate((coordinates, coordinates),axis=0) # concat to have same dimentions as corners
corners_x = base_vectors[:,0]+coordinates[:,0] # the x coord of 2 of the 4 corners of each rhombus
corners_y = base_vectors[:,1]+coordinates[:,1] # the y coord of 2 of the 4 corners of each rhombus

numRhombs = 2
numIndModes = 3

### This code will take the outputs of the make_pattern code outputs, namely 
### the coordinates of each rhombus, and two of the four corners. Then, it will 
...
def findall_corners(): #given 2 of the 4 corners of each rhombus, find the other 2
	...
	for rhomb_i in range(0, numRhombs):
		for corner_i in range(0,len(allcorners)):
			if np.round(np.linalg.norm(allcorners[corner_i,:] - coordinates[rhomb_i,:]), 5) == np.round(117.55705, 5):
				corners_sorted[count,:] = allcorners[corner_i,:]
				count = count+1
			if np.round(np.linalg.norm(allcorners[corner_i,:] - coordinates[rhomb_i,:]), 5) == np.round(161.80339887, 5):
				corners_sorted[count,:] = allcorners[corner_i,:]
				count = count+1
...
def findeach_theta(): #find the rotation, theta of each rhombus, angle between long axis dipole, and vertical
	...
	for rhomb_i in range(0, numRhombs):
		for corner_i in range(0, len(allcorners)):
			if np.round(np.linalg.norm(allcorners[corner_i,:] - coordinates[rhomb_i,:]), 5) == np.round(161.80339887, 5):
				theta = np.pi - np.arctan2(allcorners[corner_i,0] - coordinates[rhomb_i,0], allcorners[corner_i,1] - coordinates[rhomb_i,1])
...
def rotate_indmodes():
	...
	for rhomb_i in range(0, numRhombs):
		...
		Dl_vec_dirs[rhomb_i*(numRhombs+1) : rhomb_i*(numRhombs+1)+4, 0] = dip_longaxis[:,0]*np.cos(theta) - dip_longaxis[:,1]*np.sin(theta)
		Dl_vec_dirs[rhomb_i*(numRhombs+1) : rhomb_i*(numRhombs+1)+4, 1] = dip_longaxis[:,0]*np.sin(theta) + dip_longaxis[:,1]*np.cos(theta)
		
		Ds_vec_dirs[rhomb_i*(numRhombs+1) : rhomb_i*(numRhombs+1)+4, 0] = dip_shortaxis[:,0]*np.cos(theta) - dip_shortaxis[:,1]*np.sin(theta)
		Ds_vec_dirs[rhomb_i*(numRhombs+1) : rhomb_i*(numRhombs+1)+4, 1] = dip_shortaxis[:,0]*np.sin(theta) + dip_shortaxis[:,1]*np.cos(theta)
		
		Q_vec_dirs[rhomb_i*(numRhombs+1) : rhomb_i*(numRhombs+1)+4, 0] = quad[:,0]*np.cos(theta) - quad[:,1]*np.sin(theta)
		Q_vec_dirs[rhomb_i*(numRhombs+1) : rhomb_i*(numRhombs+1)+4, 1] = quad[:,0]*np.sin(theta) + quad[:,1]*np.cos(theta)
...
def final_writing():
	...
	### Writing time ###
	file = open('inputs_dimer_0210.txt','w')
